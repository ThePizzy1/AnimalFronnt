    ...
    <div class="flex">
      ...
      <div class="w-5/6 ml-auto">
        ...
        <div class="overflow-x-auto custom-scrollbar">
          ...
          >
            ...
            <tbody>
              ...
              >
                <td class="px-6 py-5 text-white font-semibold">{{ worker.firstName }}</td>
                <td class="px-6 py-5 text-gray-300">{{ worker.lastName }}</td>
                <td class="px-6 py-5 text-gray-300">{{ worker.userName }}</td>
                <td class="px-6 py-5 text-gray-300">{{ worker.email }}</td>
                <td class="px-6 py-5 text-gray-300">{{ worker.phoneNumber }}</td>
                <td
                  class="px-6 py-5 font-semibold"
                  :class="worker.roles.includes('Menager') ? 'text-emerald-400' : 'text-blue-400'"
                >
                  {{ worker.roles.includes('Menager') ? 'Manager' : 'Worker' }}
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <p class="text-gray-400 text-sm mt-5 text-center">
          Showing {{ paginatedWorkers.length }} of {{ filteredItems.length }} filtered results
          (Total: {{ items.length }})
        </p>
      </div>
    </div>
    ...
    async fetchData() {
      try {
        ...
        this.items = response.data.filter(
          (item) => item.roles.includes('Worker') || item.roles.includes('Menager')
        );
        setTimeout(() => (this.loadingError = false), 1000);
      } catch (error) {
        this.loadingError = true;
        console.error('Error fetching workers and managers:', error);
      }
    },
    nextPage() {
