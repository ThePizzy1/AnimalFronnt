    ...
    <div class="flex">
        ...
        <div class="w-5/6 text-white p-4 rounded-r-lg mr-8">
            ...
            <div class="md:w-4/4 md:ml-4 mt-4 md:mt-0 ">
                ...
                <div class="grid grid-cols-3 mr-2 ml-2 pr-2 pl-2 mt-2 gap-4 text-white">
                    <div><strong>Brand:</strong> {{ item.brandName }}</div>
                        <div><strong>Fat Content:</strong> {{ item.fatContent }}<strong> </strong></div>
                            <div><strong>Expiration Date:</strong> {{ formatDate(item.exporationDate) }}<strong> </strong></div>
                <div><strong>Age Group:</strong> {{ item.ageGroup}}</div>
                    <div><strong>Fiber Content:</strong> {{ item.fiberContent }}<strong> </strong></div>
                    <div><strong>Weight:</strong> {{ item.weight }}<strong> {{ item.measurementWeight }}</strong></div>
                <div><strong>Animal Type:</strong> {{ item.animalType}}</div>
                <div><strong>Calories Per Serving:</strong> {{ item.caloriesPerServing}}<strong> </strong></div>
                        <div><strong>Quantity:</strong> {{ item.quantity }}<strong> </strong></div>

                <div><strong>Food Type:</strong> {{ item.foodType }}</div>
                    <div><strong>Weight Per Serving:</strong> {{ item.weightPerServing }}<strong> {{ item.measurementPerServing }}</strong></div>
    ...
    methods: {
      async fetchData() {
        try {
          ...
          const response = await axios.get(`https://localhost:5001/api/animal/food/${id}`);
