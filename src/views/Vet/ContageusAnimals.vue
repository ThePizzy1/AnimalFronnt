    ...
    >
      ...
      <div class="grid grid-cols-4 gap-4 mb-4">
        <div>
          ...
          <select v-model="filters.desisseName" id="desisseName" class="text-gray-500 w-full py-2 px-3 border border-gray-300 bg-black rounded-full shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500">
        ...
        <div>
          ...
          <input v-model="filters.startTime" id="startTime" type="date" class="text-gray-500 w-full py-2 px-3 border border-gray-300 bg-black rounded-full shadow-sm focus:outline-none focus:ring-emerald-500 focus:border-emerald-500"  />
        </div>
       
        <div>
          <label for="contageus" class="block text-base font-bold mb-2">Contageus:</label>
          <select v-model="filters.contageus" id="contageus" class="text-gray-500 w-full py-2 px-3 border border-gray-300 bg-black rounded-full shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500">
            <option value="">All</option>
              ...
              <div class="relative bg-black rounded-lg shadow-sm dark:bg-gray-700">
                ...
                <form class="p-4 md:p-5">
                  <div class="grid gap-4 mb-4 grid-cols-2  sm:col-span-1">
                    ...
                    <div class="col-span-2 sm:col-span-1">
                      <label for="singleCode" class="block mb-2 text-base font-medium text-gray-900 dark:text-stone-200">Date</label>
                      <span type="text" id="singleCode"  class="bg-gray-50 border border-emerald-300 text-gray-900 text-base rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-stone-200 dark:focus:ring-primary-500 dark:focus:border-primary-500" readonly>
...
filteredItems() {
  ...
  return this.items.filter(item => {
    const idMatch = (item.animalId?.toString() || '').toLowerCase().includes(generalQuery);
    const nameMatch = (item.nameOfDesissse || '').toLowerCase().includes(generalQuery);
    const dateMatch = this.formatDate(item.date)?.toLowerCase().includes(generalQuery);
    const descriptionMatch = (item.description || '').toLowerCase().includes(generalQuery);

    return idMatch || nameMatch || dateMatch || descriptionMatch;
  });
}
